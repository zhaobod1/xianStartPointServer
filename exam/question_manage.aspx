<%@ Page Language="C#" AutoEventWireup="true" CodeFile="question_manage.aspx.cs"
    Inherits="exam_question_manage" %>
...
<body>
        ...
        <div id="content">
            ...
            <div id="infobox" style="width: 100%; position: relative; height: 95%;">
                ...
                <table id="table_list" cellpadding="0" cellspacing="0">
                    <thead>
                        <tr>
                            ...
                            <th width="40" style="border-top: none;">
                                序号
                            </th>
                           <th width="100" style="border-top: none;">
                            ...
                            <th width="400" style="border-top: none;">
                                题目
                            </th> <th width="100" style="border-top: none;">
                                试题类型
                            </th>
                            <th width="100" style="border-top: none;">
                                状态
                ...
                <table style="display: none;">
                    <tr id="template">
                        <td id="question_pk" v="" style="display: none;">
                        </td>
                        <td id="rowindex" v="" style="text-align: center; border-left: none;">
                            <input v="question_pk" name="chk_term" type="checkbox" />
                        </td>
                        <td id="RowIndex" v="" style="text-align: center;">
                        </td>
                       <td id="question_subject" v="" style="text-align: center;">
                        </td>
                        <td id="question_title" v="" style="text-align: left;">
                        </td>  <td id="question_type" v="" style="text-align: center;">
                        </td>
                        <td id="question_state" v="" style="text-align: center;">
                        </td>
                    </tr>
                </table>
            </div>
    ...
    <div id="pop_question" class="infobox" style="display: none;">
        <div class="tit">
            试题信息</div>
        ...
        <table cellpadding="0" cellspacing="0" style="background: #fff;">
            <tr>
                <td>
                    <iframe id="iframe_question" name="iframe_question" class="ifame" frameborder="0"
                        width="619px" height="396"></iframe>
                </td>
            </tr>
        </table>
    </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("#iframe_question").width(pageWidth * 80 / 100).height(pageHeight * 80 / 100);

            $("div.btn_add").click(function() {
                $("#iframe_question").attr("src", "question_add.aspx");
                $("#pop_question").css({ "left": (pageWidth - $("#pop_question").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_question").show();
            });
            $("div.btn_edit").click(function() {
                if (sel_data_pk == "") { Popalert("请选择要修改的信息！"); return; }
                $("#iframe_question").attr("src", "question_add.aspx?edit_pk=" + sel_data_pk);
                $("#pop_question").css({ "left": (pageWidth - $("#pop_question").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_question").show();
            });
            $("div.btn_look").click(function() {
                ...
                $("#iframe_question").attr("src", "question_look.aspx?edit_pk=" + sel_data_pk);
                $("#pop_question").css({ "left": (pageWidth - $("#pop_question").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_question").show();
            });
            $("div.btn_del").click(function() {
                ...
                if (confirm("确认要删除？")) {
                    $.ajax({
                        url: "getdata.aspx",
                        type: "post",
                        data: "com=del_question&del_pk=" + del_pk + "&c" + new Date().getTime(),
                        dataType: "json",
                        error: function(err) {
        ...
        function Bind() {
            ...
            $.ajax({
                url: "getdata.aspx",
                type: "post",
                data: "com=search_question&c" + new Date().getTime(),
                dataType: "json",
                error: function(err) {
                    $("#iframe_Mask").hide(); $("#divload").hide();
                    Popalert("请求的信息不存在！");
        ...
        function jumpToPage(page) {
            ...
            for (var i = startRows; i < endRows; i++) {
                if (i >= recordsCount)
                    ...
                    for (var attribute in info) {
                        ...
                        row.find("input[v='question_pk']").val(info["question_pk"]);
                    }
            ...
            for (var i = 0; i < $("#table_list tbody tr").length; i++) {
                if ($("#table_list tbody tr:eq(" + i + ") td#question_state div").length > 0) {
                    if ($("#table_list tbody tr:eq(" + i + ") td#question_state div").text() == "0")
                        $("#table_list tbody tr:eq(" + i + ") td#question_state div").html("<font color='red'>禁用</font>");
                    else if ($("#table_list tbody tr:eq(" + i + ") td#question_state div").text() == "1")
                        $("#table_list tbody tr:eq(" + i + ") td#question_state div").html("<font color='green'>启用</font>");
                }
                else {
                    if ($("#table_list tbody tr:eq(" + i + ") td#question_state").text() == "0")
                        $("#table_list tbody tr:eq(" + i + ") td#question_state").html("<font color='red'>禁用</font>");
                    else if ($("#table_list tbody tr:eq(" + i + ") td#question_state").text() == "1")
                        $("#table_list tbody tr:eq(" + i + ") td#question_state").html("<font color='green'>启用</font>");
