<%@ Page Language="C#" AutoEventWireup="true" CodeFile="rental_manage.aspx.cs" Inherits="info_rental_manage" %>
...
<body>
        ...
        <div id="content">
            ...
            <div id="infobox" style="width: 100%; position: relative; height: 95%;">
                ...
                <table style="display: none;">
                    <tr id="template">
                        <td id="rental_pk" v="" style="display: none;">
                        </td>
                        <td id="rowindex" v="" style="text-align: center; border-left: none;">
                            <input v="rental_pk" name="chk_term" type="checkbox" />
                        </td>
                      
                        <td id="rental_start_address" v="" style="text-align: center;">
                        </td>
                        <td id="rental_end_address" v="" style="text-align: center;">
                        </td>
                        <td id="rental_name" v="" style="text-align: center;">
                        </td>
                        <td id="rental_tel" v="" style="text-align: center;">
                        </td>
                         <td id="rental_number" v="" style="text-align: center;">
                        </td>
    ...
    <div id="pop_rental" class="infobox" style="display: none;">
        <div class="tit">
            长租车信息</div>
        ...
        <table cellpadding="0" cellspacing="0" style="background: #fff;">
            <tr>
                <td>
                    <iframe id="iframe_rental" name="iframe_rental" class="ifame" frameborder="0" width="619px"
                        height="396"></iframe>
                </td>
            </tr>
        </table>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("#iframe_rental,#iframe_pact,#iframe_member").width(pageWidth * 80 / 100).height(pageHeight * 80 / 100);

            $("div.btn_add").click(function() {
                $("#iframe_rental").attr("src", "rental_add.aspx");
                $("#pop_rental").css({ "left": (pageWidth - $("#pop_rental").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_rental").show();
            });
            $("div.btn_edit").click(function() {
                if (sel_data_pk == "") { Popalert("请选择要修改的信息！"); return; }
                $("#iframe_rental").attr("src", "rental_add.aspx?edit_pk=" + sel_data_pk);
                $("#pop_rental").css({ "left": (pageWidth - $("#pop_rental").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_rental").show();
            });
            ...
            $("div.btn_del").click(function() {
                ...
                if (confirm("确认要删除？")) {
                    $.ajax({
                        url: "getdata.aspx",
                        type: "post",
                        data: "com=del_rental&del_pk=" + del_pk + "&c" + new Date().getTime(),
                        dataType: "json",
                        error: function(err) {
        ...
        function Bind() {
            ...
            $.ajax({
                url: "getdata.aspx",
                type: "post",
                data: "com=search_rental&c" + new Date().getTime(),
                dataType: "json",
                error: function(err) {
                    $("#iframe_Mask").hide(); $("#divload").hide();
                    Popalert("请求的信息不存在！");
            ...
            for (var i = startRows; i < endRows; i++) {
                if (i >= recordsCount)
                    ...
                    for (var attribute in info) {
                        ...
                        row.find("input[v='rental_pk']").val(info["rental_pk"]);
                    }
