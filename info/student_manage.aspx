<%@ Page Language="C#" AutoEventWireup="true" CodeFile="student_manage.aspx.cs" Inherits="info_student_manage" %>
...
<body>
        ...
        <div class="tools" style="min-width: 740px; height: 40px; background-image: none;
            ...
            <div id="infobox" style="width: 100%; position: relative; height: 95%;">
                ...
                <table style="display: none;">
                    <tr id="template">
                        <td id="student_pk" v="" style="display: none;">
                        </td>
                        <td id="rowindex" v="" style="text-align: center; border-left: none;">
                            <input v="student_pk" name="chk_term" type="checkbox" />
                        </td>
                        <td id="RowIndex" v="" style="text-align: center;">
                        </td>
                        <td id="student_phone" v="" style="text-align: center;">
                        </td>
                       
                        <td id="student_real_name" v="" style="text-align: center;">
                        </td>
                                                               
                        <td id="student_state" v="" style="text-align: center;">
                        </td>
    ...
    <div id="pop_student" class="infobox" style="display: none;">
        <div class="tit">
            学员信息</div>
        ...
        <table cellpadding="0" cellspacing="0" style="background: #fff;">
            <tr>
                <td>
                    <iframe id="iframe_student" name="iframe_student" class="ifame" frameborder="0" width="619px"
                        height="396"></iframe>
                </td>
            </tr>
        </table>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("div.btn_add").click(function() {
                $("#iframe_student").attr("src", "student_add.aspx");
                $("#pop_student").css({ "left": (pageWidth - $("#pop_student").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_student").show();
            });
            $("div.btn_edit").click(function() {
                if (sel_data_pk == "") { Popalert("请选择要修改的信息！"); return; }
                $("#iframe_student").attr("src", "student_add.aspx?edit_pk=" + sel_data_pk);
                $("#pop_student").css({ "left": (pageWidth - $("#pop_student").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_student").show();
            });
            $("div.btn_look").click(function() {
                ...
                $("#iframe_student").attr("src", "student_look.aspx?edit_pk=" + sel_data_pk);
                $("#pop_student").css({ "left": (pageWidth - $("#pop_student").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_student").show();
            });
            ...
            $("div.btn_del").click(function() {
                ...
                if (confirm("确认要删除？")) {
                    $.ajax({
                        url: "getdata.aspx",
                        type: "post",
                        data: "com=del_student&del_pk=" + del_pk + "&c" + new Date().getTime(),
                        dataType: "json",
                        error: function(err) {
            ...
            $("div.btn_scrap").click(function() {
                ...
                if (confirm("确认要锁定所选信息吗？")) {
                    $.ajax({
                        url: "getdata.aspx",
                        type: "post",
                        data: "com=lock_student&del_pk=" + del_pk + "&c" + new Date().getTime(),
                        dataType: "json",
                        error: function(err) {
                            $("#iframe_Mask").hide(); $("#divload").hide();
                            Popalert("请求错误！");
        ...
        function Bind() {
            ...
            $.ajax({
                url: "getdata.aspx",
                type: "post",
                data: "com=search_student&student_phone=" + $("#Text1").val() + "&student_real_name=" + $("#Text2").val() + "&student_state=" + $("#Text3").val() + "&c" + new Date().getTime(),
                dataType: "json",
                error: function(err) {
                    $("#iframe_Mask").hide(); $("#divload").hide();
                    Popalert("请求的信息不存在！");
        ...
        function jumpToPage(page) {
            ...
            for (var i = 0; i < $("#table_list tbody tr").length; i++) {
                if ($("#table_list tbody tr:eq(" + i + ") td#student_state div").length > 0) {
                    if ($("#table_list tbody tr:eq(" + i + ") td#student_state div").text() == "0")
                        $("#table_list tbody tr:eq(" + i + ") td#student_state div").html("<font color='red'>待审核</font>");
                    else if ($("#table_list tbody tr:eq(" + i + ") td#student_state div").text() == "1")
                        $("#table_list tbody tr:eq(" + i + ") td#student_state div").html("<font color='green'>正常</font>");
                    else if ($("#table_list tbody tr:eq(" + i + ") td#student_state div").text() == "2")
                        $("#table_list tbody tr:eq(" + i + ") td#student_state div").html("<font color='red'>锁定</font>");                                        
                }
                else {
                    if ($("#table_list tbody tr:eq(" + i + ") td#student_state").text() == "0")
                        $("#table_list tbody tr:eq(" + i + ") td#student_state").html("<font color='red'>待审核</font>");
                    else if ($("#table_list tbody tr:eq(" + i + ") td#student_state").text() == "1")
                        $("#table_list tbody tr:eq(" + i + ") td#student_state").html("<font color='green'>正常</font>");
                    else if ($("#table_list tbody tr:eq(" + i + ") td#student_state").text() == "2")
                        $("#table_list tbody tr:eq(" + i + ") td#student_state").html("<font color='green'>锁定</font>"); 
