<%@ Page Language="C#" ValidateRequest="false" AutoEventWireup="true" CodeFile="withdraw_look.aspx.cs"
    Inherits="message_withdraw_look" %>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" border="0">
                ...
                <tr>
                 ...
                 <tr>
                    ...
                    <td>
                         <textarea class="input" style="width: 80%; height: 60px;" id="info_rem"></textarea>
                    </td>
                </tr>
            </table>
            <div style="width: 407px; margin: 30px auto;">
                 <div id="btn_cog" class="submit" style="float: left;">
                    已处理</div> 
                <div id="btn_scrap" class="rest" style="margin-left:40px; float: left;">
                    审核不通过</div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("#btn_scrap").click(function () {
                var cog_pk = "<%=edit_pk %>";               
                if (confirm("确认所选信息审核不通过？")) {
                    $.ajax({
                        url: "getdata.aspx",
                        type: "post",
                        data: "com=scrap_info&info_type=3&cog_pk=" + cog_pk + "&info_rem=" + $("#info_rem").val() + "&c=" + new Date().getTime(),
                        dataType: "json",
                        error: function (err) {
            ...
            $("#btn_cog").click(function () {
                var cog_pk = "<%=edit_pk %>";              
            
                if (confirm("确认所选信息已处理？")) {
                    $.ajax({
                        url: "getdata.aspx",
                        type: "post",
                        data: "com=cog_info&info_type=3&cog_pk=" + cog_pk + "&info_rem=" + $("#info_rem").val() + "&c=" + new Date().getTime(),
                        dataType: "json",
                        error: function (err) {
                            $("#iframe_Mask").hide(); $("#divload").hide();
                            Popalert("请求错误！");
                ...
                $.ajax({
                    ...
                    success: function(da) {
                        if (da != null) {
                            ...
                            $("#info_rem").val(textdecode(info.info_rem));
                            $("#info_title").html(info.info_title);
                            $("#info_content").html(info.info_content);
                            $("#info_obj").html("￥"+info.info_obj);
                            $("#info_state").html(info.info_state == "0" ? "<font color='red'>待处理</font>" : (info.info_state == "1" ? "<font color='green'>已处理</font>":"<font color='red'>审核失败</font>") );
