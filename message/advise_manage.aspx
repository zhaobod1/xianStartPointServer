<%@ Page Language="C#" AutoEventWireup="true" CodeFile="advise_manage.aspx.cs" Inherits="message_advise_manage" %>
...
<body>
    ...
    <div id="container">
        <div id="content"><div class="tools">                   
                        ...
                        查 看</div>
                        <div style="float: left;" class="btn_cog">
                        处 理</div>
                </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("div.btn_look").click(function() {
                ...
                $("#iframe_info").attr("src", "advise_look.aspx?edit_pk=" + sel_data_pk);
            ...
            $("div.btn_cog").click(function() {
                ...
                if ($("#table_list input[name='chk_term']:checked").length > 0) {
                    ...
                    if (sel_data_pk != "") cog_pk = "'" + sel_data_pk + "',";
                }
                if (cog_pk == "") { Popalert("请选择已处理的信息！"); return; }
                if (confirm("确认所选信息已处理？")) {
                    $.ajax({
                        url: "getdata.aspx",
                        type: "post",
                        data: "com=cog_info&info_type=2&cog_pk=" + cog_pk + "&c" + new Date().getTime(),
                        dataType: "json",
                        error: function(err) {
            ...
            $("div.btn_del").click(function() {
                ...
                if (confirm("确认要删除？")) {
                    $.ajax({
                        url: "getdata.aspx",
                        type: "post",
                        data: "com=del_info&info_type=2&del_pk=" + del_pk + "&c" + new Date().getTime(),
                        dataType: "json",
                        error: function(err) {
                            $("#iframe_Mask").hide(); $("#divload").hide();
                            Popalert("请求错误！");
        ...
        function Bind() {         
            ...
            $.ajax({
                url: "getdata.aspx",
                type: "post",
                data: "com=search_info&info_type=2&c" + new Date().getTime(),
                dataType: "json",
                error: function(err) {
                    $("#iframe_Mask").hide(); $("#divload").hide();
                    Popalert("请求的信息不存在！");
        ...
        function jumpToPage(page) {
            ...
            $("#divload").hide();
            if ("<%=pk %>" != "") {
                $("#iframe_info").attr("src", "advise_look.aspx?edit_pk=" + "<%=pk %>");
                $("#pop_info").css({ "left": (pageWidth - $("#pop_info").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_info").show();
            }
