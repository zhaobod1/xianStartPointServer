<%@ Page Language="C#" ValidateRequest="false" AutoEventWireup="true" CodeFile="message_look.aspx.cs"
    Inherits="message_message_look" %>
        ...
        $(document).ready(function() {
            ...
            if ("<%=edit_pk %>" != "") {
                $.ajax({
                    url: "getdata.aspx",
                    type: "post",
                    data: { com: "get_message_info", message_pk: "<%=edit_pk %>", c: new Date().getTime() },
                    dataType: "json",
                    error: function(err) {
                    ...
                    success: function(da) {
                        if (da != null) {
                            ...
                            $("#user_name").html(info.user_type == "1" ? (info.coach_name + " " + info.coach_phone) : (info.student_real_name + " " + info.student_phone));
                            $("#message_content").html(textdecode(info.message_content));
                            $("#message_state").html(info.message_state == "1" ? "<font color='green'>已读</font>" : "<font color='red'>未读</font>");
