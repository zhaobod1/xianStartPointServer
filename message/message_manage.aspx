<%@ Page Language="C#" AutoEventWireup="true" CodeFile="message_manage.aspx.cs" Inherits="message_message_manage" %>
...
<body>
        ...
        <div id="content">
            ...
            <div id="infobox" style="width: 100%; position: relative; height: 95%;">
                ...
                <table style="display: none;">
                    <tr id="template">
                        <td id="message_pk" style="display: none;">
                        </td>
                        <td id="rowindex" style="text-align: center; border-left: none;">
                            <input v="message_pk" name="chk_term" type="checkbox" />
                        </td>
                        <td id="message_content">
                        </td>
                        <td id="user_type" align="center">
                        ...
                        <td id="message_state" align="center">
                        </td>
    ...
    <div id="pop_message" class="infobox" style="display: none;">
        <div class="tit">
            消息详情</div>
        ...
        <table cellpadding="0" cellspacing="0" style="background: #fff;">
            <tr>
                <td>
                    <iframe id="iframe_message" name="iframe_message" class="ifame" frameborder="0" width="619px"
                        height="396"></iframe>
                </td>
            </tr>
        </table>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("#iframe_message").width(pageWidth * 80 / 100).height(pageHeight * 80 / 100);
            $("div.btn_add").click(function() {
                $("#iframe_message").attr("src", "message_add.aspx");
                $("#pop_message").css({ "left": (pageWidth - $("#pop_message").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_message").show();
            });
            $("div.btn_edit").click(function() {
                if (sel_data_pk == "") { Popalert("请选择要修改的信息！"); return; }
                $("#iframe_message").attr("src", "message_add.aspx?edit_pk=" + sel_data_pk);
                $("#pop_message").css({ "left": (pageWidth - $("#pop_message").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_message").show();
            });
            $("div.btn_look").click(function() {
                ...
                $("#iframe_message").attr("src", "message_look.aspx?edit_pk=" + sel_data_pk);
                $("#pop_message").css({ "left": (pageWidth - $("#pop_message").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_message").show();
            });
            $("div.btn_del").click(function() {
                ...
                if (confirm("确认要删除？")) {
                    $.ajax({
                        url: "getdata.aspx",
                        type: "post",
                        data: "com=del_message&del_pk=" + del_pk + "&c" + new Date().getTime(),
                        dataType: "json",
                        error: function(err) {
        ...
        function Bind() {
            ...
            $.ajax({
                url: "getdata.aspx",
                type: "post",
                data: "com=search_message&c" + new Date().getTime(),
                dataType: "json",
                error: function(err) {
                    $("#iframe_Mask").hide(); $("#divload").hide();
                    Popalert("请求的信息不存在！");
        ...
        function jumpToPage(page) {
                    ...
                    for (var attribute in info) {
                        if (isFirst) {
                            if (attribute == "user_type") {
                                row.find("#" + attribute).html(info[attribute] == "1" ? "司机" : "用户");
                            }
                            else if (info["user_type"] == "2" && attribute == "student_phone") {
                                row.find("#user_phone").html(info[attribute]);
                            }
                            else if (info["user_type"] == "2" && attribute == "student_real_name") {                          
                            ...
                            else if (attribute == "message_state") {
                                row.find("#" + attribute).html(info[attribute] == "1" ? "<font color='green'>已读</font>" : "<font color='red'>未读</font>");
                            }
                            else {
                            ...
                            if (attribute == "user_type") {
                                row.find("#" + attribute).find("div").html(info[attribute] == "1" ? "司机" : "用户");
                            }
                            else if (info["user_type"] == "2" && attribute == "student_phone") {
                                row.find("#user_phone").find("div").html(info[attribute]);
                            }
                            else if (info["user_type"] == "2" && attribute == "student_name") {
                                ...
                                row.find("#" + attribute).find("div").html(info[attribute] == "1" ? "<font color='green'>已读</font>" : "<font color='red'>未读</font>");
                            }
                            else {
                                row.find("#" + attribute).find("div").html(info[attribute]);
                            }
                        }
                        row.find("input[v='message_pk']").val(info["message_pk"]);
                    }
            ...
            $("#table_list tbody tr td").click(function() {
                ...
                if ($(this).text() == $(this).parent().find("td:eq(2)").text()) {
                    $("#iframe_message").attr("src", "message_look.aspx?edit_pk=" + sel_data_pk);
                    $("#pop_message").css({ "left": (pageWidth - $("#pop_message").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                    $("#iframe_Mask").show();
                    $("#pop_message").show();
