<%@ Page Language="C#" AutoEventWireup="true" CodeFile="withdraw_manage.aspx.cs" Inherits="message_withdraw_manage" %>
            ...
            <div id="infobox" style="width: 100%; position: relative; height: 95%;">
                ...
                <table style="display: none;">
                    <tr id="template">
                        ...
                        <td id="info_title" align="center">
                        </td>
                          <td id="info_content" align="center">
                        </td>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("div.btn_look").click(function() {
                ...
                $("#iframe_info").attr("src", "withdraw_look.aspx?edit_pk=" + sel_data_pk);
            ...
            $("div.btn_del").click(function() {
                ...
                if (confirm("确认要删除？")) {
                    $.ajax({
                        url: "getdata.aspx",
                        type: "post",
                        data: "com=del_info&info_type=3&del_pk=" + del_pk + "&c" + new Date().getTime(),
                        dataType: "json",
                        error: function(err) {
        ...
        function Bind() {         
            ...
            $.ajax({
                url: "getdata.aspx",
                type: "post",
                data: "com=search_info&info_type=3&c" + new Date().getTime(),
                dataType: "json",
                error: function(err) {
                    $("#iframe_Mask").hide(); $("#divload").hide();
                    Popalert("请求的信息不存在！");
        ...
        function jumpToPage(page) {
                ...
                if (i >= recordsCount)
                    ...
                    for (var attribute in info) {
                        if (isFirst) {
                            ...
                            row.find("#" + attribute).html(info[attribute] == "0" ? "<font color='red'>待处理</font>" : (info[attribute] == "1" ? "<font color='green'>已处理</font>" : "<font color='red'>审核失败</font>"));
                            }
                            else {
                            ...
                            row.find("#" + attribute).find("div").html(info[attribute] == "0" ? "<font color='red'>待处理</font>" : (info[attribute] == "1" ? "<font color='green'>已处理</font>" : "<font color='red'>审核失败</font>"));
                            }
                            else {
            ...
            $("#divload").hide();
            if ("<%=pk %>" != "") {
                $("#iframe_info").attr("src", "withdraw_look.aspx?edit_pk=" + "<%=pk %>");
                $("#pop_info").css({ "left": (pageWidth - $("#pop_info").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_info").show();
            }
