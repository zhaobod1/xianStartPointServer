<%@ Page Language="C#" AutoEventWireup="true" CodeFile="product_manage.aspx.cs" Inherits="product_product_manage" %>
...
<body>
        ...
        <div id="content">
            ...
            <div id="infobox" style="width: 100%; position: relative; height: 95%;">
                ...
                <table style="display: none;">
                    <tr id="template">
                        <td id="product_pk" v="" style="display: none;">
                        </td>
                        <td id="rowindex" v="" style="text-align: center; border-left: none;">
                            <input v="product_pk" name="chk_term" type="checkbox" />
                        </td>
                        <td id="RowIndex" v="" style="text-align: center;">
                        </td>                    
                        <td id="product_name" v="" style="text-align: center;">
                        </td>
                      
                        <td id="product_credit" v="" style="text-align: center;">
                        </td>
                        <td id="product_count" v="" style="text-align: center;">
                        </td>
                        <td id="product_volume" v="" style="text-align: center;">
                        </td>
    ...
    <div id="pop_product" class="infobox" style="display: none;">
        <div class="tit">
            商品信息</div>
        ...
        <table cellpadding="0" cellspacing="0" style="background: #fff;">
            <tr>
                <td>
                    <iframe id="iframe_product" name="iframe_product" class="ifame" frameborder="0" width="619px"
                        height="396"></iframe>
                </td>
            </tr>
        </table>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("#iframe_product").width(pageWidth * 80 / 100).height(pageHeight * 80 / 100);

            $("div.btn_add").click(function() {
                $("#iframe_product").attr("src", "product_add.aspx");
                $("#pop_product").css({ "left": (pageWidth - $("#pop_product").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_product").show();
            });
            $("div.btn_edit").click(function() {
                if (sel_data_pk == "") { Popalert("请选择要修改的信息！"); return; }
                $("#iframe_product").attr("src", "product_add.aspx?edit_pk=" + sel_data_pk);
                $("#pop_product").css({ "left": (pageWidth - $("#pop_product").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_product").show();
            });
            $("div.btn_look").click(function() {
                ...
                $("#iframe_product").attr("src", "product_look.aspx?edit_pk=" + sel_data_pk);
                $("#pop_product").css({ "left": (pageWidth - $("#pop_product").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_product").show();
            });
            $("div.btn_del").click(function() {
                ...
                if (confirm("确认要删除？")) {
                    $.ajax({
                        url: "getdata.aspx",
                        type: "post",
                        data: "com=del_product&del_pk=" + del_pk + "&c" + new Date().getTime(),
                        dataType: "json",
                        error: function(err) {
        ...
        function Bind() {
            ...
            $.ajax({
                url: "getdata.aspx",
                type: "post",
                data: "com=search_product&c" + new Date().getTime(),
                dataType: "json",
                error: function(err) {
                    $("#iframe_Mask").hide(); $("#divload").hide();
                    Popalert("请求的信息不存在！");
        ...
        function jumpToPage(page) {
            ...
            for (var i = startRows; i < endRows; i++) {
                if (i >= recordsCount)
                    ...
                    for (var attribute in info) {
                        ...
                        row.find("input[v='product_pk']").val(info["product_pk"]);
                    }
            ...
            for (var i = 0; i < $("#table_list tbody tr").length; i++) {
                if ($("#table_list tbody tr:eq(" + i + ") td#product_state div").length > 0) {
                    if ($("#table_list tbody tr:eq(" + i + ") td#product_state div").text() == "0")
                        $("#table_list tbody tr:eq(" + i + ") td#product_state div").html("<font color='red'>待审核</font>");
                    else if ($("#table_list tbody tr:eq(" + i + ") td#product_state div").text() == "1")
                        $("#table_list tbody tr:eq(" + i + ") td#product_state div").html("<font color='green'>正常</font>");
                }
                else {
                    if ($("#table_list tbody tr:eq(" + i + ") td#product_state").text() == "0")
                        $("#table_list tbody tr:eq(" + i + ") td#product_state").html("<font color='red'>待审核</font>");
                    else if ($("#table_list tbody tr:eq(" + i + ") td#product_state").text() == "1")
                        $("#table_list tbody tr:eq(" + i + ") td#product_state").html("<font color='green'>正常</font>");
