<%@ Page Language="C#" AutoEventWireup="true" CodeFile="sel_user.aspx.cs" Inherits="sel_user" %>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("#btn_save").click(function() {
                ...
                if (checkNode.length == 0) { Popalert("请选择用户！"); return false; }
                sel_data_pk = "{\"user\":["
                for (var i = 0; i < checkNode.length; i++) {
                    if (checkNode[i].u == "0") continue;
                    sel_data_pk += "{\"user_pk\":\"" + checkNode[i].flag + "\",";
                    sel_data_pk += "\"user_name\":\"" + checkNode[i].name + "\"}";
                    if (i < checkNode.length - 1) sel_data_pk += ",";
                }
                sel_data_pk += "]}"

                parent.set_user(sel_data_pk);
                parent.closePop();
            });
        ...
        function Bind() {
            ...
            $.ajax({
                ...
                type: "post",
                data: "com=search_org_user&c" + new Date().getTime(),
                dataType: "json",
                error: function(err) {
        ...
        function onClick(event, treeId, treeNode, clickFlag) {
            if (treeNode.u == "0") { $("a").removeClass("curSelectedNode"); sel_data_pk = ""; return }
