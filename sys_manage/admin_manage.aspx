<%@ Page Language="C#" AutoEventWireup="true" CodeFile="admin_manage.aspx.cs" Inherits="sys_manage_admin_manage" %>
...
<body>
    ...
    <div id="container">
        <div id="content">
            <table id="infobox" width="100%" border="0" cellpadding="0" cellspacing="0" style=" border:none;">
                <tr>
                    <td width="200" align="left" valign="top" style="border-top: none;">
                        <div class="zTreeDemoBackground left" style="float: left; width: 100%; border: none;">
                            <div class="tools">
                                <h3>
                                    部门列表</h3>
                            </div>
                    ...
                    <td align="left" valign="top" style="border-top: none;">
                        <div class="tools">
                            <div style="float: left;" class="btn_add">
                                新 增</div>
                            <div style="float: left;" class="btn_edit">
                                修 改</div>
                        ...
                        <table id="table_list" cellpadding="0" cellspacing="0">
                            <thead>
                                <tr>
                                    ...
                                    <th width="48" style="border-left: none;">
                        ...
                        <table style="display: none;">
                            <tr id="template">
                                <td id="user_pk" style="display: none;">
                                </td>
                                <td id="rowindex" style="text-align: center; border-left: none;">
                                    <input v="user_pk" name="chk_term" type="checkbox" />
                                </td>
                                <td id="user_login_name">
                                </td>
                                <td id="user_name">
                                </td>
                                <td id="user_ord" align="center">
                                </td>
                                <td id="user_lock" align="center">
                                </td>
                            </tr>
                        </table>
                    </td>
    ...
    <div id="pop_admin" class="infobox" style="display: none;">
        <div class="tit">
            用户帐号信息</div>
        ...
        <table cellpadding="0" cellspacing="0" style="background: #fff;">
            <tr>
                <td>
                    <iframe id="iframe_admin" name="iframe_admin" class="ifame" frameborder="0" width="619px"
                        height="396"></iframe>
                </td>
            </tr>
        </table>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("#iframe_admin").width(pageWidth * 80 / 100).height(pageHeight * 80 / 100);
            Bind();
            $("div.btn_add").click(function() {
                $("#iframe_admin").attr("src", "admin_add.aspx");
                $("#pop_admin").css({ "left": (pageWidth - $("#pop_admin").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_admin").show();
            });
            $("div.btn_edit").click(function() {
                if (sel_data_pk == "") { Popalert("请选择要修改的信息！"); return; }               
                $("#iframe_admin").attr("src", "admin_add.aspx?edit_pk=" + sel_data_pk);
                $("#pop_admin").css({ "left": (pageWidth - $("#pop_admin").width()) / 2 + "px", "top": (document.documentElement.scrollTop + 40) + "px" });
                $("#iframe_Mask").show();
                $("#pop_admin").show();
            });
            $("div.btn_del").click(function() {
                ...
                if (confirm("确认要删除？")) {
                    $.ajax({
                        url: "getdata.aspx",
                        type: "post",
                        data: "com=del_user&del_pk=" + del_pk + "&c" + new Date().getTime(),
                        dataType: "json",
                        error: function(err) {
        ...
        function Bind() {
            ...
            $.ajax({
                url: "getdata.aspx",
                type: "post",
                data: "com=search_user&c" + new Date().getTime(),
                dataType: "json",
                error: function(err) {
                    $("#iframe_Mask").hide(); $("#divload").hide();
                    Popalert("请求的信息不存在！");
            ...
            for (var i = startRows; i < endRows; i++) {
                if (i >= recordsCount)
                    ...
                    for (var attribute in info) {
                        if (isFirst) {
                            row.find("#" + attribute).html(attribute == "user_lock" ? (info[attribute] == "0" ? "是" : "否") : info[attribute]);
                        }
                        else {
                            row.find("#" + attribute).find("div").html(attribute == "user_lock" ? (info[attribute] == "0" ? "是" : "否") : info[attribute]);
                        }
                        row.find("input[v='user_pk']").val(info["user_pk"]);
                    }
        ...
        function onClick(event, treeId, treeNode, clickFlag) {
            $("#iframe_Mask").show();
            $("#divload").show();
            var org_pk = treeNode.flag;
            if (treeNode.id == "00000") org_pk = "";
            $.ajax({
                url: "getdata.aspx",
                type: "post",
                data: "com=search_user&org_pk=" + org_pk + "&c" + new Date().getTime(),
                dataType: "json",
                error: function(err) {
                    $("#iframe_Mask").hide(); $("#divload").hide();
                    $("#iframe_Mask").hide();
