<%@ Page Language="C#" AutoEventWireup="true" CodeFile="stat_order.aspx.cs" Inherits="stat_stat_order" %>
            ...
            <div class="tools" style="min-width: 740px; height: 40px; background-image: none;
                ...
                <table cellpadding="0" cellspacing="0" border="0" style="min-width: 740px;">
                    <tr>
                        ...
                        <td>
                            <input class="input" style="width: 100px;" id="Text1" type="text" />
                        </td>
                        <td align="right">
                            统计时间：
                        </td>
                        <td>
                            <input class="input" style="width: 100px;" id="Text2" type="text" />
                            至
                            <input class="input" style="width: 100px;" id="Text3" type="text" />
                        </td>
                        <td align="right">
                            状态：
                        </td>
            ...
            <div id="infobox" style="width: 100%; position: relative; height: 95%;">
                ...
                <table style="display: none;">
                    <tr id="template">
                        ...
                        <td id="coach" v="" style="text-align: center;">
                        </td>
                        <td id="order_away" v="" style="text-align: right;">
                        </td>
                        <td id="order_time" v="" style="text-align: right;">
                        </td>
                        <td id="order_away_fee" v="" style="text-align: right;">
                        </td>
                        <td id="order_time_fee" v="" style="text-align: right;">
                        </td>
                        <td id="order_far_away_fee" v="" style="text-align: right;">
                        </td>
                        <td id="order_cut_fee" v="" style="text-align: right;">
                        </td>
                        <td id="order_fee" v="" style="text-align: right;">
                        </td>
                    </tr>
                </table>
            </div>
    ...
    <script type="text/javascript">
        ...
        function Bind() {
            ...
            $.ajax({
                url: "getdata.aspx",
                type: "post",
                data: "com=stat_order&coach=" + $("#Text1").val() + "&stat_sdate=" + $("#Text2").val() + "&stat_edate=" + $("#Text3").val() + "&order_state=" + $("#Text4").val() + "&c=" + new Date().getTime(),
                dataType: "json",
                error: function(err) {
        ...
        function jumpToPage(page) {
            ...
            for (var i = 0; i < recordsCount; i++) {
                if (i >= recordsCount)
                    ...
                    for (var attribute in info) {
                        if (isFirst) {
                            if (attribute == "coach_name")
                                row.find("#coach").html(info["coach_name"]+"/"+info["coach_phone"]);
                            row.find("#" + attribute).html(info[attribute]);
                        }
                        else {
                            if (attribute == "coach_name")
                                row.find("#coach").find("div").html(info["coach_name"] + "/" + info["coach_phone"]);
            ...
            row = $("#template").clone();
            row.attr("id","hj").appendTo($("#table_list"));

            for (var i = 2; i < $("#table_list tbody tr:eq(0) td").length; i++) {
                var hj = 0;
                $("#table_list tbody tr").each(function(j) {
                    if (j == $("#table_list tbody tr").length - 1) return;
                    var v = $("#table_list tbody tr:eq(" + j + ") td:eq(" + i + ")").text();                  
                    hj = hj + parseFloat(v);
                });
                if (isFirst)
                    $("#table_list tbody tr:last td:eq(" + i + ")").text(hj.toFixed(2));
                else
                    $("#table_list tbody tr:last td:eq(" + i + ")").find("div").html(hj.toFixed(2));
            }
            if (isFirst)
                $("#table_list tbody tr:last td:eq(1)").html("合 计：");
            else
                $("#table_list tbody tr:last td:eq(1)").find("div").html("合 计：");
            if (recordsCount > 0 && isFirst) $('#table_list').flexigrid({ height: 'auto', width: 'auto', striped: true, sortable: false, onChangeSort: false });
            //$("#table_list tbody tr:last td:eq(1)").remove();
