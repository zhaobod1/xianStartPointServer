<%@ Page Language="C#" AutoEventWireup="true" CodeFile="sel_member.aspx.cs" Inherits="sel_member" %>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("div.btn_ok").click(function() {
                if ($("#table_list input[name='chk_term']:checked").length == 0 && sel_data_pk == "") { Popalert("请选择客户信息！"); return; }
                var sel_data = "{\"member\":["
                if ($("#table_list input[name='chk_term']:checked").length > 0) {
                    $("#table_list input[name='chk_term']:checked").each(function() {
                        sel_data += "{\"member_pk\":\"" + $(this).val() + "\",";
                        sel_data += "\"member_name\":\"" + $(this).parent().parent().parent().find("td:eq(2)").text() + "\"},";
                    });
                    sel_data = sel_data.substring(0, sel_data.length - 1);
                }
                else {
                    if (sel_data_pk != "") {
                        sel_data += "{\"member_pk\":\"" + sel_data_pk + "\",";
                        sel_data += "\"member_name\":\"" + sel_data_name + "\"}";
                    }
                }
                sel_data += "]}";
                parent.set_member(sel_data);
                parent.closePop();
            });
